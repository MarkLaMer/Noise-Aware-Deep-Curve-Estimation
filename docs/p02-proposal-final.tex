...
\begin{enumerate}[noitemsep]
  \item \textbf{Darkness-gated smoothness loss}: a total-variation (TV) penalty on the output---TV discourages abrupt pixel-to-pixel intensity changes---weighted per pixel by estimated input darkness (a practical proxy for local SNR), so noise is suppressed only where the signal is weak;
